...
| TD-05 | Scoring | Deterministic functions in `backend/engine/scoring.py` | Clinical arithmetic must never depend on LLM output | None — non-negotiable |
| TD-06 | Auth | Progressive: Django device token → Google OAuth (authlib) / email OTP built in `apps/accounts` | Frictionless entry + identified progress (D11) | SMS deferred to phase 2 (budget) |
...
| TD-09 | PDF summary | Server-side PDF generated in Django (library chosen at T-021 within the image budget; RTL-tested) | Shareable artifact for doctors | If RTL breaks, fallback: printable HTML → print-to-PDF |
...
- **Licence rule (Q-04):** permissive OSS (MIT / BSD / Apache-2.0 / ISC) plus LGPL libraries used unmodified;
  no GPL / AGPL / SSPL / commercial. Django and DRF are BSD-3; `psycopg` is LGPL-3.0 used unmodified.
- `main`: merged only by the owner (or on his explicit instruction). `technical-decisions.md` changes only with
  his approval.
